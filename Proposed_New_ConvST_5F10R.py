...
for file_id in range(1, folds + 1):
    ...
    Y_Train_Pred_DFrame = pd.DataFrame()
    ...
    for each_label in classes:
        ...
        Y_Train_Pred_I = logreg.predict(X_Train)
        tr_pr_end = time.time()
        ...
        Y_Train_Pred_DFrame.insert(pos, each_label, Y_Train_Pred_I)
        
        prob_label = each_label + " PROB"
        Y_Pred_Prob_DFrame.insert(pos, prob_label, Y_Pred_Prob[:, 1])

    for each_label in classes:
        X_Train[each_label] = Y_Train_Pred_DFrame[each_label].values
    ...
    conviction_matrix, use_lbls = compute_conviction(Y_Pred_DFrame, threshold = 1.2)
    ...
    for each_label in classes:
        ...
        print("Shape of Sparse Stacked Dataset : ", X_Train.shape)
        ...
        for x_lbl in drop_cols:
            X_Train[x_lbl] = Y_Train_Pred_DFrame[x_lbl].values
...
print("Total Prediction Tume : ", t_pr)
